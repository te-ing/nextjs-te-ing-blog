...
### 3. BaseModal 헤드리스 UI 컴포넌트 도입

기존 공통 모달 컴포넌트는 UI와 로직이 뒤엉켜 있어 확장성이 떨어졌다. 이를 해결하기 위해 공통 UI 요소를 분리한 **BaseModal**을 만들었다. BaseModal 컴포넌트는 헤드리스 컴포넌트 방식을 차용하여 자주 사용되지만 변경될 수 있는 Header와 Footer를 내부적으로 분리하여 쉽게 교체하거나 사용할 수 있도록 구현했다. 또한 렌더링 로직과 UI를 분리했기 때문에 필요시에는 BaseModal이 아닌 완전히 새로운 모달을 사용함으로써 더욱 유연한 모달을 만들 수 있게 되었다.
